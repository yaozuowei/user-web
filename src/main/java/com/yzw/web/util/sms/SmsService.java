...
public class SmsService {
     ...
     * @param phone 电话号码
     */
    public SmsRedis sendSmsCode(String phone) throws PlatException {
        if(!phone.matches("^1[3|4|5|6|7|8|9][0-9]{9}$")){
            throw new PlatException("手机号码格式不正确");
        }
        //判断用户输入的电话号码是否频繁发送
        if(isSendOfen(phone)){
            throw new PlatException("发送短信频繁，请稍后再试");
        }
    ...
    private boolean isSendOfen(String phone) {
        if(cacheManager.get(phone)==null) {
            return false;
        }else{
            //判断上一次记录的时间和当前时间进行对比，如果两次相隔时间小于30s，视为短信发送频繁
            SmsRedis sms= (SmsRedis) cacheManager.get(phone);
            //两次发送短信中间至少有30秒的间隔时间
    ...
    private void redisCode(SmsRedis smsRedis) {
        cacheManager.set(smsRedis.getPhone(),smsRedis,300);
    }
     ...
     * @return
     */
    public boolean validSmsCode(String phone, String code) throws PlatException{
        //取出所有有关该手机号的短信验证码
        if(cacheManager.get(phone)==null){
            throw new PlatException("短信验证失败");
        }
        SmsRedis sms= (SmsRedis) cacheManager.get(phone);
        if (sms.getCode().equals(code)){
            log.info("短信验证成功");
            //删除掉该redis
            cacheManager.del(phone);
            return true;
        }
        return false;
    }
